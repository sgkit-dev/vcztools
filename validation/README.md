...
|---|---|---|---|
| `data/small_unphased.vcz` + `data/small_phased.vcz` | 200 | ~1000 | qctool, PLINK 1.9, bgenix, REGENIE |
| `data/large_unphased.vcz` + `data/large_phased.vcz` | 5000 | ~19500 | BOLT-LMM |
...
Phenotypes (`data/<size>.pheno.tsv`) are simulated with `tstrait` on
the source tree sequence (h²=0.5, 100 causal SNPs) and shared
between the phased / unphased stores of a given size. The phenotype
...
| `make data` | Generate `data/small.vcz`, `data/large.vcz`, plus pheno files. |
...
├── data/<size>.vcz/             # generated fixtures (gitignored)
