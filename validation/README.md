...
Two synthetic fixtures, generated by `generate_data.py` from an
msprime simulation via `bio2zarr.tskit.convert`:

| Fixture | Samples | Variants | Used by |
|---|---|---|---|
| `data/small.vcz` | 200 | ~1000 | qctool, PLINK 1.9, bgenix, REGENIE |
| `data/large.vcz` | 5000 | ~19500 | BOLT-LMM |

Phenotypes (`data/<size>.pheno.tsv`) are simulated with `tstrait` on
the source tree sequence (h²=0.5, 100 causal SNPs). The phenotype
...
`generate_data.py` zeros out `call_genotype_phased` after writing
